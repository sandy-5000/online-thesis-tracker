...
export class LoginComponent implements OnInit {
	...
	isUserExisted(data: any) {
		...
		let query: string = `select name, pass, aadhar, email, count from users where roll = '${data.roll}' and pass = '${data.pass}';`;
		this.db.processQuery(query).subscribe((Data) => {
			if (Data.length != 0 && Data.length != undefined) {
				...
				sessionStorage.setItem('token', this.hash(Data[0].pass, true));
				...
				this.db.processQuery(`update site_data set value = value + 1 where parameter = 'login_count';`).subscribe(_ => {
					this.db.processQuery(`update users set count = count where roll = '${data.roll}';`).subscribe(__ => {
